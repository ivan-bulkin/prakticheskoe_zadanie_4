...
public class TicTacToe {
    ...
    //2020-12-09
    //всё это супер-пупер-отлично и я планировал сделать ещё уровни сложности, но потом прочитал ссылку Тимофея в Телеграм группы про алгорирм минимакс и понял, что надо было копать в сторону просчёта ходов на перёд, а не на различные варианты блокировки ходов человека
    //ссылка https://tproger.ru/translations/tic-tac-toe-minimax очень захватываюшее чтиво и очень хочется всё это сделать, можно начать с просчёта на три-четыре хода вперёд, но 100% на это надо часов 8-10-12 и чтобы при это ни кто не отвлекал
    //а это не цель моего обучения, да и постричься надо. Поэтому остановимся на данном варианте выполения практического задания к уроку № 4
    //Часть 1. Задания сделано: Полностью разобраться с кодом, попробовать переписать с нуля, стараясь не подглядывать в методичку;
    //с кодом разобрался, с нуля переписал(про методички вообще не знал, что они существуют), победа/ничья проверяется для поля размером до 10-ти и для заданного количества крестиков или ноликов, идущих подряд(не проверял, как будет работать, если задать 1 или 2)
    //Часть 2 и 3. Задания сделано: Попробовать переписать логику проверки победы, чтобы она работала для поля 5х5 и количества фишек 4. Очень желательно не делать это просто набором условий для каждой из возможных ситуаций;
    //Меняем static final int SIZE = 5; и static final int SKOLKO_SYMBOLOV_DLYA_POBEDI = 4; и пробуем.
    //Часть 4. Задания сделано: Доработать искусственный интеллект, чтобы он мог блокировать ходы игрока.
    //выбираем уровень сложности 2 или 3(смотри описание выше). Дальше надо использовать алгоритм минимакс. Когда-нибудь в другой раз это сделаю.
    //Единствнное, что хотел сделать, но не сделал: почему всегда первым должен ходить человек? Пусть жребий определяет, кто будет ходить первым человек или ИИ
    ...
    private static void privetstvie_programmi() {
        ...
        System.out.println("Вы можете выбрать сложность игры от 1 до 3-ти, где 1 - просто, 3 - сложно");
